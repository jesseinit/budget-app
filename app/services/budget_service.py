...
class BudgetService:
    ...
    async def get_budget_periods(
        ...
        query = query.order_by(desc(BudgetPeriod.started_at))
    ...
    async def create_budget_period(self, user_id: UUID, period_data: BudgetPeriodCreate) -> BudgetPeriod:
        """Create a new budget period - only start_date required"""
        user = await self.db.get(User, user_id)
        if not user:
            raise ValueError("User not found")

        # Calculate end_date if not provided (based on salary cycle)
        if period_data.ended_at is None:
            _, end_date = calculate_salary_period(user.salary_day, period_data.started_at)
        else:
            end_date = period_data.ended_at.replace(tzinfo=timezone.utc)

        # Check for overlapping periods
        overlapping = await self._check_overlapping_periods(user_id, period_data.started_at, end_date)
        if overlapping:
            raise ValueError("Budget period overlaps with existing period")

        # Calculate brought_forward if not provided
        if period_data.brought_forward is None:
        ...
        expected_income = period_data.expected_income or Decimal("0")
        carried_forward = period_data.carried_forward or Decimal("0")

        period = BudgetPeriod(
            user_id=user_id,
            started_at=period_data.started_at,
            ended_at=period_data.ended_at,
            expected_income=expected_income,
            brought_forward=brought_forward,
            carried_forward=carried_forward,
            status=("active" if period_data.ended_at is None else "completed"),
        )
    ...
    async def complete_budget_period(
        ...
        if period.status == "completed":
            raise HTTPException(status_code=400, detail="Budget period is already completed")

        # Use the model method to mark as completed
        period.mark_completed(ended_at=ended_at)
    ...
    async def get_transactions_for_period(self, period_id: UUID) -> List[Transaction]:
    ...
    async def recalculate_period_totals(self, period_id: UUID):
        ...
        if period:
            ...
            if period.status == "completed":
                period.carried_forward = period.calculate_carried_forward()

            await self.db.commit()

    # Helper methods
    async def _get_brought_forward_amount(self, user_id: UUID, current_start_date: datetime) -> Decimal:
        """Get amount to bring forward from previous period"""
        query = (
            select(BudgetPeriod)
            .where(and_(BudgetPeriod.user_id == user_id, BudgetPeriod.ended_at < current_start_date))
            .order_by(desc(BudgetPeriod.ended_at))
            .limit(1)
        )
        ...
        # If previous period is completed, use its carried_forward
        if previous_period.status == "completed":
            return previous_period.carried_forward

        # If not completed, calculate what would be carried forward
        return previous_period.calculate_carried_forward()

    async def _check_overlapping_periods(self, user_id: UUID, started_at: datetime, ended_at: datetime) -> bool:
        """Check if date range overlaps with existing periods"""
        query = select(BudgetPeriod).where(
            and_(
                BudgetPeriod.user_id == user_id,
                BudgetPeriod.started_at <= ended_at,
                BudgetPeriod.ended_at >= started_at,
            )
        )
    ...
    async def _get_top_expenses(self, period_id: UUID, limit: int = 5) -> List[dict]:
        ...
        return [
            {"description": t.description, "amount": float(t.amount), "date": t.transacted_at.isoformat()}
            for t in transactions
        ]

    async def rebuild_budget_period(self, period_id: UUID, user_id: UUID) -> Optional[BudgetPeriod]:
        """Rebuild a budget period: recalculate all totals and update carry forward."""
        period = await self.get_budget_period(period_id, user_id)
        if not period:
            return None
    ...
    async def rebuild_budget_periods(self, period_ids: List[UUID], user_id: UUID) -> List[BudgetPeriod]:
        """Rebuild multiple budget periods by recalculating totals and updating carry forward."""
        updated_periods = []
